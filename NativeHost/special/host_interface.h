...
enum host_mode_t
{
	...
	muxer = 1,          // Invoked as "dotnet.exe".

	apphost = 2,        // Invoked as <appname>.exe from the application base; this is the renamed "apphost.exe".

	split_fx = 3,       // Invoked as "corehost.exe" for xunit scenarios. Supported for backwards compat for 1.x apps.
...
struct host_interface_t
{
	...
	const char_t* fx_dir;
	const char_t* fx_name;
	const char_t* deps_file;
	...
	size_t host_mode;
	const char_t* tfm;
	const char_t* additional_deps_serialized;
	const char_t* fx_ver;
	strarr_t fx_names;
	strarr_t fx_dirs;
	strarr_t fx_requested_versions;
	strarr_t fx_found_versions;
	const char_t* host_command;
	const char_t* host_info_host_path;
	const char_t* host_info_dotnet_root;
	const char_t* host_info_app_path;
